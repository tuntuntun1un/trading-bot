import requests
import json
from flask import Flask, request, jsonify

app = Flask(__name__)

# ===== НАСТРОЙКИ TELEGRAM (ЗАМЕНИ НА СВОИ!) =====
TELEGRAM_BOT_TOKEN = "8634619027:AAGx24rMqSYjiUEPgnlovkrS8Dffe4LGZyU"  # Вставь сюда токен
TELEGRAM_CHAT_ID = "720615262"              # Вставь сюда свой ID
SECRET = "my_secret_2025"                     # Пароль для проверки сигнала

# ===== ФУНКЦИЯ ОТПРАВКИ В TELEGRAM =====
def send_telegram(message):
    url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage"
    payload = {
        'chat_id': TELEGRAM_CHAT_ID,
        'text': message,
        'parse_mode': 'HTML'
    }
    try:
        r = requests.post(url, json=payload)
        print(f"Telegram ответ: {r.status_code}")
    except Exception as e:
        print(f"Ошибка отправки в Telegram: {e}")

# ===== ОБРАБОТЧИК WEBHOOK ОТ TRADINGVIEW =====
@app.route('/webhook', methods=['POST'])
def webhook():
    try:
        # Получаем данные от TradingView
        data = request.get_json()
        print(f"📡 Получен сигнал: {data}")

        # Проверяем секретный пароль
        if data.get('secret') != SECRET:
            print("❌ Неверный секрет!")
            return jsonify({"error": "Unauthorized"}), 401

        # Извлекаем сигнал и цену
        signal = data.get('signal')
        price = data.get('price')

        if signal in ['buy', 'sell']:
            # Формируем красивое сообщение
            msg = f"🔔 <b>НОВЫЙ СИГНАЛ ОТ TRADINGVIEW!</b>\n\n"
            msg += f"Направление: <b>{'ПОКУПКА 🟢' if signal == 'buy' else 'ПРОДАЖА 🔴'}</b>\n"
            msg += f"Цена: <b>{price} USDT</b>\n"
            msg += f"Пара: XRPUSDT\n"
            msg += f"Время: {request.headers.get('X-Forwarded-For', 'N/A')}"
            
            # Отправляем сообщение в Telegram
            send_telegram(msg)
            print("✅ Сообщение отправлено в Telegram")
            return jsonify({"status": "ok"}), 200
        else:
            print("❌ Неверный сигнал")
            return jsonify({"error": "Invalid signal"}), 400

    except Exception as e:
        print(f"❌ Критическая ошибка: {e}")
        return jsonify({"error": str(e)}), 500

# ===== ЗАПУСК СЕРВЕРА =====
if __name__ == "__main__":
    print("🚀 TELEGRAM-БОТ ЗАПУЩЕН!")
    print("Жду сигналов от TradingView...")
    app.run(host='0.0.0.0', port=5002)
